...
//	ICE_VDC_VEHICLE_TYPE_NORMAL_CAR			= 1,			//!<  普通汽车
//	ICE_VDC_VEHICLE_TYPE_MINIBUS			= 2,			//!<  面包车
//	ICE_VDC_VEHICLE_TYPE_BUS				= 3,			//!<  大客车
//	ICE_VDC_VEHICLE_TYPE_BOXTRUCK			= 4,			//!<  箱式货车
//	ICE_VDC_VEHICLE_TYPE_TRUCK				= 5,			//!<  大货车
...
typedef struct 
{
	...
	ICE_S32 s32TripWireEventCode;							//!< 绊线事件码（0：无事件；1：缓存图像；2：绊线触发；3：绊线触发，但使用当前图像抠图）
...
typedef struct 
{
	...
	ICE_U32  u32PlateWidth;  								//!< 实际车辆宽度	
	ICE_U32  u32ExtSnapPixel;								//!< 抓拍图外osd像素高度
	ICE_Phone_Mac_List *pPhoneMacList;						//!< 接收终端mac 列表
	ICE_CHAR  cPhoneMacListReserve[4];  					//!< mac列表64位指针兼容预留
	ICE_U32	 u32PictureHashID;							//!< 图片哈希ID
	ICE_U32  u32Encrypt;									//!< 车牌号码是否加密
	ICE_CHAR  cReserve[52];									//!< 预留
	ICE_S32 s32TripWireEventCode;							//!< 绊线事件码（0：无事件；1：缓存图像；2：绊线触发；3：绊线触发，但使用当前图像抠图）
	ICE_S32  s32VehicleIntensity;							//!< 车身亮度
	ICE_CHAR cVehicleType[12];								//!< 车身颜色(字符串方式) 
	ICE_VCR_VEHICLECOLOR_E enVehicleColor;					//!< 车身颜色
	ICE_VDC_VEHICLETYPE_E s32VehicleType;					//!< 车辆类型
	ICE_SYSTEMTIME_S  stCapTime;    						//!< 抓拍时间     			   						
	ICE_S32    s32HaveCloseUp;     							//!< 是否有特写图像     						
