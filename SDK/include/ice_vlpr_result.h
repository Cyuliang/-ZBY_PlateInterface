...
typedef enum
{
	...
	ICE_PLATE_COLOR_GREENBLACK	= 6,	//!<  绿底黑字
	ICE_PLATE_COLOR_GREENYELLOW	= 7		//!<  绿黄底黑字(大型新能源车辆)
}ICE_PLATECOLOR_E;
...
#define ICE_VLPR_PLATE_CHAR_NUM		10								//!<  车牌字符数    
#define ICE_VLPR_PLATE_BUFLEN		16                              //!<  车牌号最大值  

//! 车牌真实区域输出，4个顶点坐标，为4位定标；
typedef struct
{
...
{                                                                       //!<  车牌字符个数；
	...
	ICE_S32             s32PlateContrast;								//!<  车牌对比度；                                                        
	ICE_S8				astReserve0[40];								//!<  预留字段
	...
	ICE_U8				u8PlateColorRate;								//!<  颜色匹配程度；
	ICE_S8				s8CharNum;
	ICE_RECT_4P_S		stRealPlateRect;								//
	ICE_S8				astReserve;										//!<  预留参数
	ICE_S8              sPlateFmt;										//!<  0:默认 100:UTF8 101:BIG5 102:GBK
	ICE_U32             u32FrameId;
}ICE_VLPR_OUTPUT_S;
